...
output_folder_actual_gd_rel_path = "../Data/Simple Dispatch Outputs/Actual Scenario"
output_folder_rel_path = "../Data/Simple Dispatch Outputs/Counterfactual Scenario/Fuel Price Metrics"
...
for nerc_region in nerc_region_all:
    ...
    # EIA 923
    eia923_fname = 'EIA923_Schedules_2_3_4_5_M_12_'+str(run_year)+'_Final_Revision.xlsx' 
    ...
    temp_orispl_prices[[i for i in range(1, 13)]] = orispl_prices[range(1, 13)].apply(
        lambda x: adjust_to_2006_1_real_dollars(x, run_year, x.name)) # adjusts based on year and month
    
    ## create a dataframe to hold fuel_price_metrics: number of units, average, min, max, stadard deviation
    ...
    # Repeat the 'average', 'stadard deviation', 'min', and 'max' columns with suffixes from 1 to 12
    ...
    for fuel_type in unique_fuel_types:
        if fuel_type == 'ng':
            for purchase_type in purchase_types: # iterate over all contract types, same method as in last 'else' statement (that one is more readable)
                ...
                ## calculate metrics (number of units, average, min, max, stadard deviation)
                ...
                fuel_price_metrics.loc[fuel_price_metrics_row, 'total_stadard_deviation'] = temp.stack().std(axis=0, skipna=True)
        else:
            # mask for units that have matching fuel type
            mask = temp_orispl_prices["fuel"] == fuel_type
            # mask for fuel price metrics index
    ...
    fuel_price_metrics.to_csv('actual_fuel_price_metrics_'+nerc_region+'_'+str(run_year)+'.csv', index=False)